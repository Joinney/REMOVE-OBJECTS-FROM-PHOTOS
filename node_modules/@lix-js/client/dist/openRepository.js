...
export async function openRepository(url, args) {
    ...
    const lazyFS = typeof experimentalFeatures.lazyClone === "undefined"
        ? freshClone
    ...
    const remove = (filepath) => isoGit.remove({
        fs: withProxy({
            nodeishFs: rawFs,
            verbose: debug,
            description: "remove",
    ...
    async function emptyWorkdir() {
        ...
        const notIgnored = (await Promise.all(listing.map((entry) => isoGit.isIgnored({ fs: rawFs, dir, filepath: entry }).then((ignored) => {
    ...
    async function statusList(statusArg) {
    ...
    async function status(filepath) {
        ...
        const maybeStatusEntry = statusList[0] || [filepath, "unknown"];
    ...
    return {
        ...
        _remove: remove,
        // @ts-ignore
        _isoCommit: ({ author: overrideAuthor, message }) => isoGit.commit({
            fs: withProxy({
                nodeishFs: rawFs,
                verbose: debug,
        ...
        async commit({ author: overrideAuthor, message, include, }) {
            if (include) {
                const additions = [];
                const deletions = [];
        ...
        async pull(cmdArgs) {
            ...
            if (experimentalFeatures.lazyClone) {
                console.warn("enableExperimentalFeatures.lazyClone is set for this repo but pull not fully implemented. disabling lazy files");
                await emptyWorkdir();
