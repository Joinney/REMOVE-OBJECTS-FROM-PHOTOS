...
const { walk, TREE, WORKDIR, STAGE, isIgnored, } = isoGit;
 ...
 */
export async function statusList({ fs, dir = "/", gitdir = join(dir, ".git"), ref = "HEAD", filepaths = ["."], filter, sparseFilter, // experimental, not yet exposed as lix api!
cache, includeStatus = [], addHashes = false, }) {
    try {
        return await walk({
            fs,
            cache,
            ...
            map: async function (filepath, [head, workdir, stage]) {
                ...
                if (entry.headOid === entry.workdirOid && entry.workdirOid === entry.stageOid) {
                    if (includeStatus.includes("unmodified") ||
