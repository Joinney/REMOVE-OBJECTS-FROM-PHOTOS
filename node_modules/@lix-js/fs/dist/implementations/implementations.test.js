...
describe.skipIf(process.platform === "win32")("node fs", async () => {
    ...
    // @ts-ignore
    const isNodeFs = true;
    await runFsTestSuite("node fs", tempDir, fs, isNodeFs);
});
describe("memory fs", async () => {
    const fs = createNodeishMemoryFs();
    await runFsTestSuite("memory fs", "", fs);
});
const runFsTestSuite = async (name, tempDir, fs, isNodeFs = false) => {
    ...
    test("recursive mkdir", async () => {
        expect(await fs.mkdir(`${tempDir}/home/user1/documents/`, { recursive: true })).toMatch(/^.*\/home\/?$/);
        expect(await fs.mkdir(`${tempDir}/home/user1/downloads/`, { recursive: true })).toMatch(/^.*\/home\/user1\/downloads\/?$/);
    ...
    test("file r/w", async () => {
        ...
        expect(await fs.readFile(`${tempDir}/home/user1/documents/file1`, { encoding: "utf-8" })).toEqual(textInFirstFile);
    ...
    test("symlink", async () => {
        await fs.symlink(`${tempDir}/home/./user1/../user1/documents///./file1`, `${tempDir}/file1.link`);
        await fs.symlink(`${tempDir}/file3`, `${tempDir}/file3.link`);
        await fs.symlink(`${tempDir}/home/user1`, `${tempDir}/user1.link`);
        expect(await fs.readFile(`${tempDir}/file1.link`, { encoding: "utf-8" })).toEqual(textInFirstFile);
    ...
    test("readlink", async () => {
        expect(await fs.readlink(`${tempDir}/file1.link`)).toEqual(`${tempDir}/home/./user1/../user1/documents///./file1`);
    ...
    test("stat/lstat", async () => {
        ...
        expect([stats.user1.isFile(), stats.user1.isDirectory(), stats.user1.isSymbolicLink()]).toEqual([false, false, true]);
        expect([stats.file1.isFile(), stats.file1.isDirectory(), stats.file1.isSymbolicLink()]).toEqual([false, false, true]);
        expect([stats.file2.isFile(), stats.file2.isDirectory(), stats.file2.isSymbolicLink()]).toEqual([true, false, false]);
    ...
    describe("throw errors", async () => {
        ...
        test("writeFile", async () => {
            await expect(async () => await fs.readFile(`${tempDir}/home/dne/file`, { encoding: "utf-8" })).rejects.toThrow(/ENOENT/);
        });
        test("readFile", async () => {
            await expect(async () => await fs.readFile(`${tempDir}/home/dne`, { encoding: "utf-8" })).rejects.toThrow(/ENOENT/);
            await expect(async () => await fs.readFile(`${tempDir}/home/user1`, { encoding: "utf-8" })).rejects.toThrow(/EISDIR/);
        });
        test("readdir", async () => {
            await expect(async () => await fs.readdir(`${tempDir}/home/dne`)).rejects.toThrow(/ENOENT/);
            await expect(async () => await fs.readdir(`${tempDir}/home/user1/documents/file1`)).rejects.toThrow(/ENOTDIR/);
        });
    });
    test("rm", async () => {
        await expect(async () => await fs.rm(`${tempDir}/home/user1/documents/`)).rejects.toThrow(/EISDIR/);
        await fs.rm(`${tempDir}/home/user1/documents/file1`);
        await expect(async () => await fs.readFile(`${tempDir}/home/user1/documents/file1`, { encoding: "utf-8" })).rejects.toThrow(/ENOENT/);
        await fs.writeFile(`${tempDir}/home/user1/documents/file1`, textInFirstFile);
