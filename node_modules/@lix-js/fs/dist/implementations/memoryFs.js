...
export function createNodeishMemoryFs() {
    ...
    return {
        writeFile: async function (path, data, options) {
            ...
            const parentDir = fsMap.get(dirName);
        ...
        symlink: async function (target, path) {
            ...
            const targetInode = fsMap.get(normalPath(target));
            const parentDir = fsMap.get(getDirname(path));
            if (fsMap.get(path))
